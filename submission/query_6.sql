...
INSERT INTO
    amaliah21315.hosts_cumulated WITH old_events AS (-- Subquery to select all records from "hosts_cumulated" for a specific date
        SELECT
            *
        FROM
    ...
    new_events AS (
        ...
        GROUP BY
            e.host,
            -- Gets web events grouped by the host
            CAST(date_trunc('day', e.event_time) AS DATE) -- Gets web events grouped by the event day
    )
SELECT
    -- Main SELECT statement to combine old and new event data
    COALESCE(oe.host, ne.host) AS host,
    -- Select host from old events or new events (preferring old events if available)
    CASE
        WHEN oe.host_activity_datelist IS NOT NULL
        AND ne.event_date IS NOT NULL THEN ARRAY [ne.visits] || oe.visits -- Append only if both are not null
    ...
    CASE
        WHEN oe.host_activity_datelist IS NOT NULL
        AND ne.event_date IS NOT NULL THEN ARRAY [ne.event_date] || oe.host_activity_datelist -- Append only if both are not null
...
FROM
    old_events oe FULL
    OUTER JOIN new_events ne ON oe.host = ne.host -- To include all old and new events
INSERT INTO user_devices_cumulated WITH yesterday AS (
  ...
  today AS (
    ...
    SELECT user_id,
      ...
      COUNT(1)
    FROM bootcamp.web_events as w -- Join in the devices table to get both browser_type and user_id
...
SELECT COALESCE(y.user_id, t.user_id) AS user_id,
  ...
  CASE
    -- Old user case
    WHEN y.dates_active IS NOT NULL THEN ARRAY [t.event_date] || y.dates_active -- If the user was active today, add the date to the array
...
FROM yesterday y -- Ensure both tables are joined on the user_id and browser_type fields
